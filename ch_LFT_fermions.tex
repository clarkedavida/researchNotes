...
\subsection{Highly improved staggered fermions}\index{action!HISQ}
...
\begin{equation}\begin{aligned}
  \Lagr_\text{clover}^\text{tad}=~&\bar{\psi}(x)\psi(x)\\
    &-\frac{\kappa}{u_0}\frac{1}{a}\bar{\psi}(x)\sum_\mu
     \left[(\id+\gamma_\mu)U_\mu(x)\psi(x)
           +(\id-\gamma_\mu)U_\mu^\dagger(x)\psi(x+a\hat{\mu})\right]\\
...
%\section{Chemical potential and the sign problem}
