...
\begin{equation}\label{eq:fermionIntegrationMeasure}
...
fields given in \chref{ch:preliminaries}. Working in the continuum 
...
\begin{proof} From the definitions~\eqref{eq:deffermgf} and
  ...
  $$
  Let us now apply \thmref{thm:fermgf} to the RHS and carry
...
\end{equation} 
This second term is called the {\it Wilson term}\index{Wilson!term}. 
This formulation
...
that remove doublers. {\it Staggered fermions}~\cite{kogut_hamiltonian_1975}
...
therefore {\it rooting}\cite{marinari_monte_1981}. 
The idea is that $\det D^{\rm stag}$ is
...
\end{equation}
The hope is that the off-diagonal part of $D^{\rm stag}$ is irrelevant
in the RG sense,
so that $D^{\rm stag}$ becomes block-diagonal in the continuum
limit.
