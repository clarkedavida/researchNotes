...
\section{A very brief recap of some SR}
...
general coordinate transform, we include boosts, like
\begin{equation}\label{eq:boostRot}
x^\mu\to x'^{\mu} = \Lambda_\nu^\mu x^\nu + a^\mu,
\end{equation}
...
term quadratic in $\phi$ has a corresponding ``force" linear in $\phi$. This can
be interpreted as a linear ``restoring force" that pushes particles into and out
of existence. Whatever form this ``restoring force" has in reality, we know that
close enough to equilibrium, we can approximate it linearly.
\end{itemize}
...
Lorentz-covariant solution is
\begin{equation}
\phi(x)=\int\frac{\dd[3]{p}}{(2\pi)^32E_p}
\left(a_pe^{ipx}+a^\dagger_pe^{-ipx}\right).
\end{equation}
