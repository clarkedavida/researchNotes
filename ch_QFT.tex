...
text book I've encountered is by Schwartz~\cite{schwartz_quantum_2014}, and it is
...
so perhaps it is not surprising that Galilean physics is recovered.
...
all inertial frames, and a consequence is that the {\it line element}\index{line element}
\begin{equation}
...
\end{equation}
The first term is clearly the kinetic energy term, or equivalently, it is the
...
Moreover, I do not know a clean way to define the symbol 
...
\end{equation}
where we have introduced the {\it energy-momentum tensor}\index{energy-momentum tensor} $\Theta^{\mu\nu}$. We see that spacetime translations induce four
...
clearly a corresponding conservation law.




\subsection{The Dirac equation}\index{Dirac!equation}

As mentioned during the mnemonic history, the Klein-Gordon equation has the 
issue that its probabilities become time-dependent.\footnote{This is eventually
resolved by reinterpreting the probability current as a charge-probability
current.} This was resolved by Dirac, who sought an equation linear in its
differentials, like the Schr\"odinger equation is.
A thorough examination of the Dirac, Klein-Gordon, and Schr\"odinger equations
can be found in Ref.~\cite{tsutsui:2025}. 

According to Dirac, if we follow the form of Schr\"odinger, we would write
\begin{equation}\label{eq:protoDirac}
i\partial_0\psi=i\left(\alpha_k\partial_k-i\beta m\right)\psi,
\end{equation}
where we won't specify what kind of objects $\alpha_k$ or $\beta$ are.
We have also switched to $\psi$ for the field, anticipating that it will have a
fundamentally different structure than the $\phi$.
Applying another $\partial_0$, using a plane-wave Ansatz for $\psi$, and
reorganizing, we get
\begin{equation}
E^2\psi=\left(\alpha_i^2p_i^2+m^2\beta^2
+\frac{1}{2}\sum_{i,j\suchthat i\neq
j}\left(\alpha_i\alpha_j+\alpha_j\alpha_i\right)p_ip_j
+m\left(\alpha_i\beta+\beta\alpha_i\right)p_i\right)\psi.
\end{equation}
Thus in order to satisfy the dispersion relation, we must have
\begin{equation}\label{eq:alphaBetaDirac}
\alpha_i^2=\beta^2=1,~~~~~
\alpha_i\alpha_j+\alpha_j\alpha_i=0,~~~~
\alpha_i\beta+\beta\alpha_i=0
\end{equation}
for each $i\neq j$. The equations \eqref{eq:alphaBetaDirac} suggest
a nontrivial algebra, and objects of any algebra can always be represented by
matrices. Let its dimension be $n$. The third of these constraints tells us
\begin{equation}
\tr \alpha_i = \tr \alpha_i\beta^2 = -\tr \beta\alpha_i\beta=-\tr\alpha_i,
\end{equation}
which means the matrices $\alpha_i$ are traceless. Moreover the first constraint
tells us the eigenvalues are $\pm1$. The only way to get traceless matrices with
such eigenvalues is if $n$ is even. Setting $n=2$, one might use the Pauli
matrices. But then only $\beta\propto\id$ is linearly independent of these,
which commutes with all of them, violating the third constraint. 

Setting $n=4$ works. For example one can take the representation
\begin{equation}
\alpha_i
  =\left(\begin{array}{cc}
       &  \sigma_i       \\ 
...
\beta
  =\left(\begin{array}{cc}
    ...
    \end{array}\right),~~~~~
\end{equation} 
which fulfills all the relations \eqref{eq:alphaBetaDirac}. This is
called the {\it Dirac
representation}\index{Dirac!representation}\index{representation!Dirac}. 
At this stage,
one typically organizes these in terms of the matrices\index{matrix!$\gamma$}
$\gamma^0\equiv\beta$ and $\gamma^i\equiv\beta\alpha_i$. In terms of these
matrices, \equatref{eq:protoDirac} becomes the familiar Dirac equation
\begin{equation}
\left(i\gamma^\mu\partial_\mu-m\right)\psi=0.
\end{equation}
In the Dirac representation, the $\gamma$s can be written 
\begin{equation}
\gamma^0
  =\left(\begin{array}{cc}
       \id_2& \\ 
...
The Dirac equation tells us that the $\psi$ must have four components. These are
called {\it spinors}\index{spinor}. The index running over these four components
is called the {\it Dirac} or {\it spin}
index\index{index!Dirac}\index{index!Spin}. Thus each $\gamma$ has two free spin
indices, and each spinor has one. The $\gamma$s are differentiated by their
Lorentz index. In a sense, all this extra internal structure for $\psi$
ultimately follows from formulating Schr\"odinger in a way that respects SR.

The term $(i\gamma^\mu\partial_\mu-m)$ is often referred to as the {\it Dirac
operator}\index{Dirac!operator} or {\it Dirac matrix}. The partial derivative
changes under Lorentz transformations, so to keep the Dirac equation Lorentz
invariant, it must be that the spinor transforms non-trivially.
If we write the spinor transformation as
\begin{equation}
\psi'(x')=U(\Lambda)\psi(x),
\end{equation}
then the requirement that the Dirac equation be Lorentz invariant forces
the $\gamma$s to transform as
\begin{equation}\label{eq:gammaLorentz}
U^{-1}\gamma^\mu U=\Lambda^\mu_\nu\gamma^\nu,
\end{equation}
i.e. they are Lorentz vectors. 

The $\gamma$s appear when one works with spin-$1/2$ representations of
the Lorentz group. Here we collect some of their algebraic properties.
We see 
\begin{equation}
  \left\{\gamma^\mu,\gamma^\nu\right\}=2g^{\mu\nu}\id_4
\end{equation}
and
\begin{equation}
\left(\gamma^0\right)^2=-\left(\gamma^i\right)^2=-\id_4.
\end{equation}
The representation of the Lorentz algebra is
\begin{equation}
  \sigma^{\mu\nu}=\frac{i}{4}\left[\gamma^\mu,\gamma^\nu\right].
\end{equation}

The Hermitian conjugate of the Dirac equation can be written
most succinctly if one defines the {\it adjoint} Dirac 
spinor\index{adjoint!spinor} 
\begin{equation}
  \bar{\psi}\equiv\psi^\dagger\gamma^0.
\end{equation}
The adjoint spinor allows us to write down {\it bilinears}\index{bilinear}
of the form $\bar{\psi}\Gamma\psi$ for a generic matrix $\Gamma$ in Dirac space. 
...
%\index{spinor}
%the above generators is called a {\it Dirac spinor}. 
...
\end{equation}
and we can then write the Dirac equation as 
\begin{equation}
  (i\slashed{\partial}-m)\psi=0.
\end{equation}
Here are some derivable algebraic properties of the $\gamma$s:
\begin{proposition}{}{gammatech}
Gamma matrix identities in \dimens{4}:
\begin{center}\begin{tabular}{lrcl}
1. &$\tr\left[\text{odd no. of $\gamma$s}\right]$ &=& $\zd_4$;\\
2. &$\slashed{a}\slashed{b}$ &=& $-\slashed{b}\slashed{a}-2(ab)\id_4$;\\
3. &$\tr\gamma^\mu\gamma^\nu$ &=& $-4g^{\mu\nu}\id_4$;\\
4. &$\gamma_\mu\gamma^\mu$&=& $-4\id_4$. 
\end{tabular}\end{center}
\end{proposition}

Solutions to the Dirac equation can be expressed as four linearly independent
spinor solutions times a plane wave. Rather than write these explicitly, we will
call the spinor parts $u_1$, $u_2$, $v_1$, and $v_2$, so that a general solution
will have the form
\begin{equation}
\psi(x)=\int\frac{\dd[3]{p}}{(2\pi)^32E_p}
\sum_{r=1,2}\left(a_r(p)u_r(p)e^{ipx}+b_r(p)v_r(p)e^{-ipx}\right).
\end{equation}

\subsection{Spin, helicity, and chirality}\index{spin}

The solutions to the Dirac equation 
turn out to describe fermions. In particular, the spin operator
is
\begin{equation}
S^i=\frac{1}{2}\left(\begin{array}{cc}
                 \sigma^i  &   \\
                   &\sigma^i  
           \end{array}\right)
=-\frac{i}{2}\alpha_1\alpha_2\alpha_3\alpha_i.
\end{equation}
It can be shown that, e.g. picking momentum along the $z$-direction, the spinors
$u_1$, $u_2$, $v_1$, and $v_2$ are eigenvectors of $S_z$ with eigenvalues $\pm
1/2$, i.e. they are representing spin-1/2 particles.

The {\it helicity}\index{helicity} of a particle is the projection of its spin
onto its momentum,
\begin{equation}
h\equiv \frac{2p_i S^i}{|\vec{p}|}.
\end{equation}
Helicity has eigenvalues $\pm 1$ and it is conserved during dynamical processes.
However it is not Lorentz invariant.

The matrix
\begin{equation}
  \gamma_5=i\gamma^0\gamma^1\gamma^2\gamma^3
\end{equation}
is the {\it chirality}\index{chirality} operator, which is Lorentz invariant.
It allows us to define left-handed and right-handed projection operators
...
\end{equation}
For massless particles, chirality and helicity coincide. The chirality of a
particle is intrinsic to it, but the helicity depends on the reference frame, as
the spin is intrinsic but the momentum is frame dependent. 

We close this section with some identities related to chiral projection
operators:
\begin{proposition}{}{projection}
Projection operator identities:
\begin{center}\begin{tabular}{lrcl}
1. &$\gamma_5^2$ &=& $\id_4$;\\
2. &$\left\{\gamma^\mu,\gamma_5\right\}$ &=& $\zd_4$;\\
4. & $P_R^2$ &=& $P_R$;\\
5. & $P_L^2$ &=& $P_L$;\\
5. & $\gamma^\mu P_L$ &=& $P_R \gamma^\mu$;\\
6. & $\gamma^\mu P_R$ &=& $P_L \gamma^\mu$.
\end{tabular}\end{center}
\end{proposition}


\subsection{Discrete symmetries}\label{sec:discreteSymm}

The first two discrete symmetries we discuss are examples of Lorentz
transformations. In particular, the {\it parity}\index{parity} and {\it
time-reversal}\index{time reversal} transformations can be written as
\begin{equation}
...
T=\left(\begin{array}{cc}
           ...
           \end{array}\right).
\end{equation} 
From \equatref{eq:gammaLorentz} one sees that
\begin{equation}
U(P)=\gamma^0
\end{equation}
and thus
\begin{equation}
U(P)^{-1} \gamma^5 U(P)=-\gamma^5,
\end{equation}
i.e. $\gamma_5$ transforms as a pseudoscalar.
...
\end{equation}
for fermion flavors $f$ and $g$. In \tabref{tab:discreteSymm}, we collect the
transformation types of various bilinears.
...
\begin{equation}
  \Lagr=\frac{1}{2}\left(\partial_\mu\phi\right)^2
       ...
       \equiv\frac{1}{2}\left(\partial_\mu\phi\right)^2
        +V(\phi)
\end{equation}
is invariant under the orthogonal group O$(N)$. 
This is the Lagrangian of the\index{sigma model}
...
Let us try to gain some geometric intuition for this phenomenon. 
...
Let us introduce the $s$ quark, assuming it has the same mass as the
...
\index{baryon!number}
{\it baryon number}\footnote{You can see why it is called baryon number: 
...
\end{equation}
with the {\it pion decay constant}\index{decay constant!$\pi$} $f_\pi$ given by
\begin{equation}
...
\section{Exactly solvable models}

There is a handful of QFTs that can exactly solved. These QFTs are useful
as proving grounds for various tools; for example they have been employed to
test Lefschetz thimbles, and multi-point Pad\'e approximants.
They also often exhibit some features of more physically realistic theories;
for example they may be asymptotically free like QCD is.

\subsection{Thirring model}\label{sec:Thirring}\index{Thirring model}
...
\psi\right),
\end{equation}
where here $\gamma^\mu$ are \dimens{2} gamma matrices.
This describes a Dirac field with a self-interaction.
What is special about the massless Thirring model is that it is 
...
\subsection{Gross-Neveu model}\label{sec:GNmodel}\index{Gross-Neveu model}
...
To see $\ket{\theta}$ is an eigenstate of $H$, note
\begin{equation}\begin{aligned}
H\ket{\theta}&=\sum_n e^{-in\theta}H\ket{n}\\
&=\sum_{n,k} e^{-in\theta}\ket{k}\bra{k}H\ket{n}\\
&=\sum_{n,k} e^{-in\theta}\ket{k}f(|k-n|)\\
&=\sum_{k,m} e^{-i(k-m)\theta}\ket{k}f(|m|)\\
&=\ket{\theta}\sum_{m} e^{im\theta}f(|m|).
\end{aligned}\end{equation}
...
that at $N_c=\infty$ the $\eta'$\index{meson!$\eta$} mass is related to 
the topological susceptibility through
\begin{equation}
  m_{\eta'}^2+m_\eta^2-2m_K=\frac{4N_f\chi_Q}{f_\pi^2},
\end{equation}
where $m_\eta$ is the $\eta$ mass, $m_K$ is the mass of the
kaon\index{meson!$K$}, 
$N_f$ is the number of fermion flavors, and $f_\pi$ is the
pion decay constant\index{decay constant!$\pi$}. This {\it Witten-Veneziano
mechanism}\index{Witten-Veneziano mechanism} can be used to explain the 
...
As discussed in \secref{sec:magnets},
the magnetic moment\footnote{This is the dipole contribution to the
...
\begin{figure}
\centering
\includegraphics[width=0.3\linewidth]{figs/HVP.pdf}
\caption{HVP contributions to $a_\mu$. {\it Top}: Quark-line
...
\begin{figure}
  \centering
  \includegraphics[width=\linewidth]{figs/HIC.pdf}
  \caption{Cartoon of an HIC. Baryons are depicted in the last panel, but of
course mesons are also produced. See text for explanation.
Figure taken from Ref.~\cite{valois_thermal_2025}.}
  \label{fig:HIC}
\end{figure}


\section{Heavy-ion collisions}\index{collision!heavy-ion}

At the moment, heavy-ion collisions (HICs) are the primary direct method to
experimentally probe strongly interacting matter, especially when one is
interested in high temperatures and densities. They are the stage on which 
some of the ``real physics" we discuss later will be investigated; therefore we
discuss briefly the physics of an HIC and familiarize ourselves with some jargon
used in the HIC community.
We follow two very nicely written 
theses~\cite{valois_thermal_2025,kleinehanding_magnetic_2026}.

In an HIC, two heavy ions, e.g. gold ions, are collided at relativistic speeds.
The region where the ions overlap is extremely hot and dense for a very brief
time, and we infer characteristics of the overlap region from the ultimate
products of the collision. 
In \figref{fig:HIC} we show a cartoon of a HIC. In the left-most frame, we see a
collision from the center-of-mass frame of the identical heavy ions;
alternatively you can think of it as the rest frame of the experimenters.
Protons and neutrons are distinguished by having different colors, and the blue
arrow shows the initial motion of the heavy ions.
The two heavy ions look like pancakes--this is because the heavy
ions move at relativistic speeds w.r.t. the center-of-mass frame, and hence
this frame sees their shapes get severely Lorentz contracted. The incoming heavy
ions are depicted as being slightly off center. This is an example
of an {\it off-central}\index{collision!off-central} collision: it has
non-zero impact parameter. {\it Central collisions}\index{collision!central} are
head-on collisions and correspondingly have zero impact parameter.

In the next frame, we see the initial stage of the collision. The heavy ions
have moved through each other. Nucleons from the region where the heavy ions
overlapped interact strongly with each other. The nucleons from this overlap
region, i.e. the nucleons from one heavy ion that interact with nucleons from
the other, are called the {\it participants}\index{participant}.
Remaining nucleons are the {\it spectators}\index{spectator}.
A small red region indicates what
is sometimes called the {\it fireball}\index{fireball}; in \figref{fig:HIC} only
participants are touching the fireball. For a very brief time,
the fireball is expected to be in the QGP phase. As time goes on, the fireball
expands and cools.

In the next stage of the collision, the fireball is well described by
relativistic hydrodynamics. This is the hydrodynamic stage. Once the fireball
has become cool enough, quarks and gluons must bind into hadrons, a stage called
hadronization, shown in the last panel. After the system has hadronized and
inelastic collisions between the hadrons has ceased, we reach a stage called
{\it chemical freeze-out}\index{freeze-out!chemical}, where the relative abundances of
different hadron species becomes fixed. Shortly thereafter, the system attains
{\it kinetic freeze-out}\index{freeze-out!kinetic}, where hadrons can be
considered as non-interacting hadronic gas.

\subsection{The hydrodynamic stage}

In general, the collisions are going to be off-central. This means that the
overlap region in general has an almond shape, i.e. there is a large geometric
anisotropy. The medium at this stage is still strongly interacting, so it makes
sense to model it as a fluid. Thinking classically for a moment, the outside
of the almond is at some pressure $p_{\rm out}$ and the pressure inside the
almond has pressure $p_{\rm in}$. Euler's equations tell us that the particle
motion is determined by the pressure gradient. An almond has a short axis and a
long axis, so geometrically, the pressure gradient is larger along the shorter
axis. Thus the expansion of the fluid will be faster along the short axis than
the long axis, and hence more particles should be detected along the short-axis
direction.

In relativistic hydrodynamics, Euler's equations generalize to
\begin{equation}\label{eq:EMconserve}
\partial_\mu \Theta^{\mu\nu}=0
\end{equation}
with energy-momentum tensor\index{energy-momentum tensor}
\begin{equation}
\Theta^{\mu\nu}\equiv\left(\epsilon+p\right)u^\mu u^\nu-pg^{\mu\nu},
\end{equation}
where $u^\mu$ is the four-velocity of the fluid and $\epsilon$ is the energy
density. Thus it generalizes to energy-momentum conservation.
Let $\phi$ measure the angle in the collision
plane with $\phi=0$ and $\phi=\pi$ being the two ends of the short axis. Then
the above argument translates to an expectation that most particles will be
detected at $\phi=0$ and $\phi=\pi$. More quantitatively,
\begin{equation}
\dv{N}{\phi}=N_0\left(1 +2v_1\cos(\phi) + 2v_2 \cos(2\phi) + \cdots\right)
\end{equation}
The coefficient $v_2$ is called the {\it elliptic flow}\index{elliptic flow}.
This coefficient can be predicted in relativistic hydrodynamics, and it agrees
extremely well with experiment, validating its use. If, for example, the medium
were non-interacting instead, the expansion would be the same in all directions,
and we would find $v_i\approx 0$.

Now working in the framework of relativistic hydrodynamics, we would like to
derive an interesting property of the expanding medium; namely the expansion
follows lines of constant $s/n_B$. To see this, we will need a couple reminders
about relativity and thermodynamics.

First, since $\dd\tau^2=g_{\mu\nu}\dd{x}^\mu\dd{x}^\nu$, we have
\begin{equation}\label{eq:unormal}
u_\mu u^\mu=g_{\mu\nu}\dv{x^\mu}{\tau}\dv{x^\nu}{\tau}
=\frac{\dd\tau^2}{\dd\tau^2}=1,
\end{equation}
i.e. the four-velocities are normalized. Hitting both sides with
$\partial_\nu$ and doing a little index gymnastics reveals 
\begin{equation}\label{eq:updvu}
u_\nu\partial_\mu u^\nu=0.
\end{equation}
We also want to exploit the
Gibbs-Duhem relation\index{Gibbs-Duhem relation}
\begin{equation}
\epsilon+p=Ts+\mu_Bn_B,
\end{equation}
where $n_B$ is the net-baryon number density, $\mu_B$ is the corresponding
chemical potential, and $s$ is the entropy density. Additionally we want the first
law of thermodynamics written in terms of intensive variables only,
\begin{equation}
\dd\epsilon=T\dd s+\mu_B\dd n_B.
\end{equation}
Finally it is useful to define currents
\begin{equation}
j_B^\mu\equiv n_Bu^\mu,~~~~~~s^\mu\equiv su^\mu,
\end{equation}
and the fact that net-baryon number is conserved, set by the initial conditions,
can be cast as
\begin{equation}
\partial_\mu j_B^\mu=0.
\end{equation}

On to the derivation. We start by contracting \equatref{eq:EMconserve} with
$u_\nu$
\begin{equation}\label{eq:snBderiv1}
0=u_\nu\partial_\mu\Theta^{\mu\nu}
=u_\nu\partial_\mu\left(\left(\epsilon+p\right)u^\mu u^\nu-pg^{\mu\nu}\right)
=\partial_\mu\left(\left(\epsilon+p\right)u^\mu\right)-u^\mu\partial_\mu p,
\end{equation}
where we used Eqs. \eqref{eq:unormal} and \eqref{eq:updvu}.
Conservation of net-baryon number lets us write
\begin{equation}\label{eq:snBderiv2}
\partial_\mu u^\mu=-\frac{u^\mu\partial_\mu n_B}{n_B}.
\end{equation}
Expanding the first term in \equatref{eq:snBderiv1} and substituting
in \equatref{eq:snBderiv2}, the first law, and Gibbs-Duhem yields 
\begin{equation}
0=Tu^\mu\left(\partial_\mu s-\frac{s}{n_B}\partial_\mu n_B\right)
=Tu^\mu n_B\partial_\mu\left(\frac{s}{n_B}\right). 
\end{equation}
Hence,
\begin{equation}
u^\mu \partial_\mu\left(\frac{s}{n_B}\right)=0. 
\end{equation}
$In fact, plugging net-baryon number conservation into the above
$equation one even finds
$\begin{equation}
$\partial_\mu s^\mu=0,
$\end{equation}
$i.e. the entropy is also conserved.

\section{Electromagnetic fields in QCD}\index{magnetic field}\label{sec:EMQCD}

Strong magnetic fields manifest in the off-central HICs discussed above, and
they are also present in neutron stars and the primordial universe. In this
section we discuss QCD with electromagnetic fields.

\subsection{Time-independent background EM fields}

We begin by treating the magnetic field as a time-independent background field,
i.e. it is externally fixed. We start with infinite volume. The quarks carry
electric charge, so the covariant derivative becomes
\begin{equation}\label{eq:covDerivEM}
\DmuEM=\partial_\mu+iA_\mu+ie_f\AmuEM,
\end{equation}
where $e_f$ is the electric charge for quark flavor $f$. This ensures the
$\U(1)$ gauge symmetry when including the background field. The strong coupling
is the same for all quarks, so it is absorbed in $A_\mu$. 
Since gluons do not couple to photons, the pure gluon part of the QCD action will
remain the same, i.e. it will be the standard Yang-Mills action. The fermionic
part will be the same, except for the replacement $D_\mu\to\DmuEM$. One also
needs a kinetic term for the photons
\begin{equation}
\frac{1}{4}\int\dd[4]{x}F^{\rm EM}_{\mu\nu} F^{\rm EM,\mu\nu}
\end{equation} 
Since the EM field is treated as background, the photon kinetic term can be
directly factored out of the path integral, as the path integral runs over
$\SU(3)$ configurations only.

We will consider homogeneous fields.\footnote{Inhomogeneous fields can be
realized by having an external current that varies within the volume.} The
medium responds to external currents as
\begin{equation}
j_\mu^{\rm tot}=j_\mu^{\rm ext}+\ev{j_\mu}.
\end{equation}
The second term on the RHS encodes dynamical QCD effects reacting to the
external field; since the dynamical photon factors out of the path integral,
there is no back-reaction. The classical electric and magnetic fields can be
computed from the field-strength tensor as
\begin{equation}
E_i=F^{\rm EM}_{0i},~~~~~~~B_j=\frac{1}{2}\epsilon_{ijk}F^{\rm EM}_{jk}.
\end{equation}
