...
$e^-$ and $\nu_e$. Experimentally it is roughly $1\times10^{-5}$ GeV$^{-2}$.},
...
matrix. For example one finds for any flavor $f$\footnote{In practical
calculations, traces are estimated using noise vectors, as discussed in
\secref{sec:noiseVector}. Then one has to be careful to distinguish between the
average used to estimate the trace and the expectation value over gauge
configurations, which is what is indicated by the $\ev{}$ brackets in these
equations. Furthermore, when using noise vectors to estimate quantities of the
form $\tr\ev{}^2$, one has to be again careful: The square is computed by the
product of two sums, and one can show that including diagonal terms, i.e.
product terms originating from the same random vector, is a biased estimator.
Removing the diagonal terms eliminates this bias.\index{bias}}
...
In the last sections, we expressed the pressure as a sum over cumulants,
c.f. \equatref{eq:pxpac}, and provided some formulae to compute the second-order 
(and higher) cumulants. What is missing is the leading term in this expansion,
i.e. $\chi^{uds}_{000}$. In this section we discuss a popular strategy for
obtaining this quantity on the lattice.

Recall that the symmetric energy-momentum\index{energy-momentum tensor} 
tensor $\Theta^{\mu\nu}$ has components describing
energy density, energy-current density, momentum density, and moment-current
density. We think about the $\Theta^{\mu\nu}$ describing strongly interacting matter.
Working in the comoving frame, the elements $\Theta^{0i}$ are zero.
Assuming the matter is an ideal fluid, shear terms $\Theta^{ij}$, $i\neq j$, drop
out, and further using the isotropy of the fluid, we are left with 
\begin{equation}
\Theta^{\mu\nu}=\diag\left(\epsilon,P,P,P\right),
\end{equation}
where $\epsilon\equiv U/V$ is the energy density and $P$ is the pressure. 
The {\it trace anomaly}\index{trace anomaly} or {\it interaction
measure}\index{interaction measure} is simply the trace,
\begin{equation}
\Theta^{\mu\mu}=\epsilon-3P.
\end{equation}
Using appropriate powers of the temperature to make everything unitless, we find
\begin{equation}\label{eq:thetaPressureDE}
\frac{\Theta^{\mu\mu}}{T^4}=T\pdv{P/T^4}{T}.
\end{equation}
We have a couple expectations of $\Theta^{\mu\mu}$. To leading order in
high-temperature perturbation theory~\cite{Kapusta:1979fh},
\begin{equation}
\frac{\Theta^{\mu\mu}}{T^4}=\frac{b_0}{3}\left(1+\frac{5N_f}{12}\right)g^4(T)+\order{g^5},
\end{equation}
which means $\Theta^{\mu\mu}$ should vanish at $T=\infty$. At temperatures below
$\Tpc$, the HRG model predicts a sharp decline with decreasing temperature. 
This can be seen from Eqs. \eqref{eq:pressrelQM} and \eqref{eq:edensityQM};
in particular the $K_2$ cancels out, and $T\to0$ sends the argument of $K_1$ to
infinity, suppressing it exponentially.

Equation \eqref{eq:thetaPressureDE} is the fundamental relation to compute
the leading term of the pressure on the lattice. In particular we see that we
can integrate this equation starting from a reference temperature $T_0$ to obtain
\begin{equation}
\frac{P(T)}{T^4}-\frac{P(T_0)}{T_0^4}
=\int_{T_0}^T \dd{T'}\frac{\Theta^{\mu\mu}(T')}{T'^5}.
\end{equation}
If $T_0$ is low enough, this can be estimated from HRG. I have also seen cases
where the trace anomaly is estimated to be zero at $T_0$.

To get the trace anomaly from the lattice, we follow a common strategy
exemplified e.g. in Ref.~\cite{bazavov_equation_2014}. Here it is useful
to write the QCD partition function directly as
\begin{equation}
  Z=\int\DD\bar\psi\DD\psi\DD U e^{-\beta S_G+S_F},
\end{equation}
i.e. we have not integrated out the fermion measure
\eqref{eq:fermionIntegrationMeasure}, and we have factored $\beta$ out
of the gluonic part of the action. Using $V=a^3N_s^3$, $T=1/aN_\tau$,
and $P\approx (T/V)\log Z$, we find
\begin{equation}
\frac{\Theta^{\mu\mu}}{T^4}=-\frac{a}{VT^3}\dv{\log Z}{a}.
\end{equation}
Modern simulations generally run directly at physical quark mass. In simulations
with an up, down, and strange quark only, this often means that $am_s(a)$ is tuned 
according to the physical value of $m_s$, and then $am_u(a)$ and
$am_d(a)$ are chosen so that $am_q(a)/am_s(a)=m_q/m_s$.
Thus all bare quark masses have the same dependence on $a$, up to an overall
factor. Nothing else depends on on $a$, so the derivative comes out to be
\begin{equation}
\dv{\log Z}{a}=-\ev{\dv{(\beta S_G-S_F)}{a}}.
\end{equation}

The trace anomaly can then be split into parts coming from the gluonic and
fermionic pieces of the total action. Moreover the derivative w.r.t. $a$ can be
recast as a derivative w.r.t. $\beta$. The gluonic part is then straightforward:
\begin{equation}\label{eq:TAGbare}
\frac{\Theta^{\mu\mu,{\rm bare}}_G}{T^4}=-R_\beta N_\tau^4 \ev{s_G},
\end{equation}
where $s_G$ is the gluonic action density, $s_G=S_G/N_s^3N_\tau$, and
\begin{equation}
R_\beta\equiv-a\dv{\beta}{a}.
\end{equation}
The action density has an additive lattice divergence. This
additive divergence is temperature-independent; therefore the trace anomaly
can be renormalized by subtracting $s_G$ evaluated at the same $\beta$ but
at $T\approx 0$, i.e. from some ensemble with a very large temporal extension.
Thus the renormalized gluonic contribution reduces to\footnote{We emphasize 
that in \equatref{eq:TAG} one takes the $N_\tau$ belonging to the $T>0$
ensemble.}
\begin{equation}\label{eq:TAG}
\frac{\Theta^{\mu\mu}_G}{T^4}=R_\beta
N_\tau^4\left(\ev{s_G}_0-\ev{s_G}_T\right).
\end{equation}

For the fermionic contribution, note
\begin{equation}
\dv{S_F}{a}
=\frac{\dd}{\dd a}\sum_{q,x} \bar{q}(x)\left(\slashed{D}+m_q\right)q(0)
=\sum_{q,x} \bar{q}q(x) \dv{m_q}{a}.
\end{equation}
Keeping in mind we are on the LCP described above and applying the chain rule,
we find
\begin{equation}
\dv{S_F}{a}
=-\frac{R_\beta R_m}{a}\sum_{q,x} m_q \bar{q}q(x), 
\end{equation}
where
\begin{equation}
R_m\equiv\frac{1}{m_s}\dv{m_s}{\beta}.
\end{equation}
With the convention $\Theta^{\mu\mu}=\Theta^{\mu\mu}_G+\Theta^{\mu\mu}_F$
and renormalizing by again subtracting a temperature-independent additive
divergence, we find
\begin{equation}
\frac{\Theta^{\mu\mu}_F}{T^4}=-R_\beta R_mN_\tau^4
\sum_q m_q\left(\ev{\bar{q}q}_0-\ev{\bar{q}q}_T\right),
\end{equation}
where again the condensates $\ev{\bar{q}q}$ have been averaged over
the 4-volume.

The function $R_\beta$ must be obtained using some reference scale as an
intermediary. In Ref.~\cite{bazavov_equation_2014}, they use $r_1$, so
that
\begin{equation}
R_\beta=\frac{r_1}{a}\left(\dv{(r_1/a)}{\beta}\right)^-1.
\end{equation}
This is a matter of scale setting: Other options like $f_K$ are possible.
Meanwhile $R_m$ is essentially determined when tuning $am_s(a)$ to its
physical value. 


%\subsection{QCD material parameters}
%
%Besides the EoS, it is informative to look at various {\it material
%parameters}\index{material parameter}, i.e. quantities that characterize various
%properties of a studied material, for strongly interacting matter.
%
%The speed of sound at fixed control parameter $X$ is defined as
%\begin{equation}
%  c_X^2=\left(\pdv{P}{\epsilon}\right)_X.
%\end{equation}
%In a simple (1+1)-dimensional, isotropic model of a heavy ion collision, 
...
%interaction term. This means it is non-renormalizable, so it is not expected
...
endpoint belonging to the \dimens{3}, $\Z_2$ universality class, as shown in the rear 
plane of \figref{fig:pdiag}.
...
$\mu\approx m_p$ to the {\it nuclear matter density}\index{nuclear matter density} $n_0\approx 0.16$ fm$^{-3}$. This jump is discontinuous, and
so the expectation is that transition is first-order, with $n_B$
playing the role of an order parameter.

To better understand this jump, we repeat the argument of
Ref.~\cite{halasz_phase_1998}. Imagine a grand canonical system, perhaps
something like the overlap region of two colliding heavy ions. We consider
this system in contact with a baryonic reservoir, which in this example,
would be the non-overlapping part of the ions. Isolating the effect of baryonic
particles only, the grand partition function
reads
\begin{equation}
Z=\sum_\alpha\exp{-\frac{E_\alpha-\mu N_\alpha}{T}},
\end{equation}
where the sum runs over states, $E$ is the energy, $N$ the net-baryon number,
and $\mu$ the baryon chemical potential. In the limit $T\to0$, the lowest-energy
state dominates $Z$, which at $\mu=0$ is the ground state $E=N=0$. At fixed $T$,
the only way to change the system's state is to increase $\mu$ until a baryon
is pulled into the system. Let
\begin{equation}
\mu_0\equiv\min_\alpha E_\alpha/N_\alpha
\end{equation}
and let $\alpha=0$ be a state that satisfies this minimum. If $\mu<\mu_0$, then
$E_0-\mu N_0>0$, and due to the $T\to0$ limit, the $\alpha=0$ state is
exponentially suppressed, i.e. forbidden. Correspondingly, when $\mu>\mu_0$, 
the argument of the exponential becomes positive, and is thus favored over the
ground state. Therefore
\begin{equation}
n(\mu) = 
\begin{cases} 
    0 & \text{if } \mu < \mu_0 \\ 
    \text{positive} & \text{if } \mu \geq\mu_0, \\
\end{cases}
\end{equation}
i.e. $n(\mu)$ exhibits a discontinuity at $\mu_0$.

We continue our discussion of qualitative features of the phase diagram.
...
\section{Electromagnetic fields}\label{sec:latEM}
In \secref{sec:EMQCD} we discussed adding electromagnetic fields in the
continuum theory. We found the classical electric and magnetic fields to be
recoverable from the field-strength tensor as
\begin{equation}
E_i=\FEM_{0i},~~~~~~~B_j=\frac{1}{2}\epsilon_{ijk}\FEM_{jk}.
\end{equation}
Working in a Euclidean metric, the fields are rotated to $A_4=iA_0$.
This renders the electric field purely imaginary, and it is not directly
accessible on the lattice. The magnetic field does not have this issue.
We follow Refs.~\cite{brandt_thermal_2023,valois_thermal_2025,kleinehanding_magnetic_2026}.

Targeting lattice calculations, we first work in the continuum, restricting to
finite spatial volume $V=L_1L_2L_3$, $-L_i/2\leq x_i\leq L_i/2$. We start as
usual with periodic BCs. So for instance
\begin{equation}
\AmuEM(x+L_i\hat{i})=\AmuEM(x)
\end{equation}
Let us first consider the magnetic flux through the 
$x_i-x_j$ plane at $x_k=0$. From Stokes's theorem, we get
\begin{equation}\begin{aligned}
\Phi_{ij}&=\int\dd{x_i}\dd{x_j}B_k(x_i,x_j)\\
         &=\int\dd{x_i}\left(\AEM_i(x_i,-L_j/2)-\AEM_i(x_i,L_j/2)\right)\\
&~~~~~~+\int\dd{x_j}\left(\AEM_j(L_i/2,x_j)-\AEM_j(-L_i/2,x_j)\right).
\end{aligned}\end{equation}
So if we continue with our setup, the periodic BCs guarantee zero flux.
To get a nonzero flux, we instead allow periodicity up to a gauge
transformation:
\begin{equation}
\AmuEM(x+L_i\hat{i})=\AmuEM(x)+\partial_\mu\phi_i(x).
\end{equation}
This BC is sometimes called a {\it twist}\index{twist} since the
potential changes through one revolution of the torus.
With the twisted BC, the magnetic flux becomes
\begin{equation}
\Phi_{ij}=\phi_i(x+L_j\hat{j})-\phi_i(x)-\phi_j(x+L_i\hat{i})+\phi_j(x).
\end{equation}

Moving on to the fermion fields, we need to make sure their BCs are consistent
with the gauge field BCs; in particular, we want the covariant derivative
\eqref{eq:covDerivEM} to remain periodic. Suppressing all spacetime components
except those on the plane, this leads to BCs
\begin{equation}\begin{aligned}
\psi_f(x_i,x_j+L_j)&=e^{-ie_f\phi_j(x_i)}\psi_f(x_i,x_j)\\
\psi_f(x_i+L_i,x_j)&=e^{-ie_f\phi_i(x_j)}\psi_f(x_i,x_j),
\end{aligned}\end{equation}
where $e_f$ is the electric charge of flavor $f$.
This turns out to place a constraint on $\phi(x)$. To see why, consider two paths
on the $x_i-x_j$ plane following its edges. The phases the fermions pick up
when moving along these paths are 
\begin{equation}\begin{aligned}
x\to x+L_i\to x+L_i+L_j &:~~~ e^{-ie_f\left(\phi_j(x+L_i)+\phi_i(x)\right)}\\
x\to x+L_j\to x+L_i+L_j &:~~~ e^{-ie_f\left(\phi_i(x+L_j)+\phi_j(x)\right)}.\\
\end{aligned}\end{equation}
To ensure the fermion field is single-valued, it must be that
\begin{equation}
e_f\left(\phi_i(x+L_j\hat{j})-\phi_i(x)-\phi_j(x+L_i\hat{i})+\phi_j(x)\right)=2\pi
N_b^f,
\end{equation}
where $N_b^f\in\Z$. 
It follows that
\begin{equation}
\Phi_{ij}=\frac{2\pi}{e_f}N_b^f.
\end{equation}
This condition expresses {\it magnetic flux
quantization}\index{flux quantization}; we learn that in a finite volume, only
discrete values of the magnetic flux are accessible.

\subsection{Lattice regularization}

We have hitherto discussed a continuum formulation for time-independent
background fields. The process of introducing these fields on a lattice is
similar as for gluon fields, discussed in \secref{sec:latreg}; namely we define
{\it photonic} link variables\index{link!photonic} by\footnote{In the case of
the gluonic links, we absorbed the coupling and a factor $i$ in $A_\mu$. Here
we leave these factors explicit so that we can work with directly with real
$B$.}
\begin{equation}
u_\mu^f(x)=e^{-iae_f\AmuEM(x)}.
\end{equation}
For this discussion of discretization, we are going to use the Wilson
Dirac operator \eqref{eq:WilsonDirac}. The operator with our background fields
becomes
\begin{equation}\label{eq:WilsonDirac}
  D(x,y)^f_{\alpha\beta c_1c_2}
  =\left(m^f+\frac{4}{a}\right)\delta_{\alpha\beta}
                               \delta_{c_1c_2}
                               \delta_{xy}
   -\frac{1}{2a}\sum_{\mu=\pm 1}^{\pm 4}(\id-\gamma_\mu)_{\alpha\beta}
      U_\mu(x)_{c_1 c_2}u_\mu^f(x)\delta_{x+a\hat{\mu},y},
\end{equation}
i.e. it amounts to a simple dressing of the gluon field $U_\mu$.\footnote{The
same would hold true for staggered fields.} Again, the
$u_\mu$ factor out of the path integral, so they do not need to be sampled,
which makes the calculation easier.

Given an external field, what remains is to find the $u_\mu$ that generates it.
We can orient $B$ along the $z$-axis WLOG. Since $B$ is the curl of $\AmuEM$,
this can be accomplished with the gauge choice
\begin{equation}
\AEM_2(x)=Bx_1,~~~~~~\AEM_\mu(x)=0
\end{equation}
for $\mu\neq 2$. It follows that the links are
\begin{equation}
u^f_2(x)=e^{-iae_fBx_1},~~~~~u^f_\mu(x)=1
\end{equation}
for $\mu\neq 2$. With this gauge choice, these links are periodic only up to a
gauge transformation, as they were in the continuum case; namely
\begin{equation}\label{eq:utwisted}
u^f_2(L_1/2,x_2)=e^{iae_fBL_1}u_2(-L_1/2,x_2).
\end{equation}
In terms of $\AmuEM$,
\begin{equation}\begin{aligned}
\AEM_2(L_1 / 2, x_2)&=\frac{1}{2} B L_1=\AEM_2(-L_1 / 2, x_2)+B L_1, \\
\AEM_2(x_1, L_2 / 2)&=\AEM_2(x_1,-L_2 / 2) .
\end{aligned}\end{equation}
The first expression can be recast in the form of a gauge transformation:
\begin{equation}
\AEM_2(x_1+L_1 / 2, x_2)=\AEM_2(x_1, x_2)+\frac{1}{e_f} \frac{\partial}{\partial
x_2}(e_f B L_1 x_2).
\end{equation}

\begin{figure}
\centering
\includegraphics[width=0.5\linewidth]{figs/Bxform.pdf}
\caption{
Representation of gauge transform for $u^f_\mu$ fields on the lattice in the
$x_1$-$x_2$ plane. Red arrows represent transformed $u_2^f$ links while green
arrows indicate transformed $u_1^f$ links. We see that the gauge transformation
on the $u_2$ links affects the $u_1$ links on the penultimate slice.
Image taken from Ref.~\cite{brandt_thermal_2023}.
}
\label{fig:Bxform}
\end{figure}

To make implementing $u_\mu$ easier for the lattice, we are now going to deform
slightly from this gauge to achieve PBC. This amounts to cancelling the extra
factor in \equatref{eq:utwisted}. Based on the preceding discussion, we define
\begin{equation}
f(x_1, x_2)\equiv\left\{\begin{array}{cc}
e_f B L_1 x_2 & \text { if } x_1=L_1 / 2 \\
0 & \text { otherwise }
\end{array}\right.
\end{equation}
If we then define our gauge transformation that deforms away from the original
gauge by
\begin{equation}
\Omega(x_1,x_2)=e^{if(x_1,x_2)},
\end{equation}
we find the extra term will indeed be cancelled:
\begin{equation}
\begin{aligned}
u_2^{f\,\prime}\left(L_1 / 2, x_2\right) & =\Omega\left(L_1 / 2, x_2\right) u_2\left(L_1 / 2, x_2\right) \Omega^*\left(L_1 / 2, x_2+a\right) \\
& =e^{i e_f B L_1 x_2} e^{i a e_f B L_1 / 2} e^{-i e_f B L_1\left(x_2+a\right)} \\
& =e^{-i a e_f B L_1 / 2} \\
& =u^f_2\left(-L_1 / 2, x_2\right).
\end{aligned}
\end{equation}
As sketched in \figref{fig:Bxform}, we see this transformation affects $u_1^f$
links on $L_1/2-a$ slice by
\begin{equation}
\begin{aligned}
u_1^{f\,\prime}\left(L_1 / 2-a, x_2\right) & =\Omega\left(L_1 / 2-a, x_2\right) u_1\left(L_1 / 2-a, x_2\right) \Omega^*\left(L_1 / 2, x_2\right) \\
& =e^{-i e_f B L_1 x_2}.
\end{aligned}
\end{equation} 
Hence the total prescription for our periodic EM links yielding $B$ in the
$x_3$-direction is
\begin{equation}
\begin{aligned}
& u_1^f\left(x_1, x_2\right)= \begin{cases}e^{-i e_f B L_1 x_2} & \text { if } x_1=L_1 / 2-a, \\
1 & \text { otherwise },\end{cases} \\
& u_2^f\left(x_1, x_2\right)=e^{i e_f B x_1},-L_1 / 2 \leq x_1<L_1 / 2, \\
& u_3^f\left(x_1, x_2\right)=1, \\
& u_4^f\left(x_1, x_2\right)=1 .
\end{aligned}
\end{equation}
If we demand also periodicity in the $x_2$-direction for the $u_1^f$ links, we
see
\begin{equation}
\begin{aligned}
u^f_1\left(L_1 / 2-a, L_2 / 2\right) & =u^f_1\left(L_1 / 2-a,-L_2 / 2\right) \\
e^{-i e_f B L_1 L_2 / 2} & =e^{i q B L_1 L_2 / 2} \\
e^{-i e_f B L_1 L_2} & =1,
\end{aligned}
\end{equation}
which implies
\begin{equation}
B=\frac{2 \pi N_b}{e_f L_1 L_2}, \quad N_b \in \mathbb{Z} .
\end{equation}
We thus recover flux quantization on the lattice.
This gives us all the tools we need to implement Wilson fermions in QCD with a
background, homogeneous magnetic field.
