...
We follow Ref.~\cite{kleinehanding_magnetic_2026}.
...
\Phi_{ij}&=\int\dd{x_i}\dd{x_j}B_k(x_i,x_j)\\
         &=\int\dd{x_i}\left(A^{\rm EM}_i(x_i,-L_j/2)-A^{\rm
EM}_i(x_i,L_j/2)\right)\\
&~~~~~~+\int\dd{x_j}\left(A^{\rm EM}_j(L_i/2,x_j)-A^{\rm EM}_j(-L_i/2,x_j)\right).
\end{aligned}\end{equation}
...
\begin{equation}
A^{\rm EM}_2=Bx_1,~~~~~~A^{\rm EM}_\mu=0
\end{equation}
for $\mu\neq 2$. It follows that the links are
\begin{equation}
u^f_2=e^{-iae_fBx_1},~~~~~u^f_\mu=1
\end{equation}
...
\end{equation}
% Deform gauge choice to get periodicity
% complete periodicity yields again flux quantization
% You can check the EM plaquette remains invariant
% Take the picture from Dean's paper
% Try to understand/reconcile these two ways of seeing quantization? 
