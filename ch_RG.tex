...
Z_N=\sum_{n=0}^M g(n)e^{-\beta n\epsilon}.
\end{equation}
Introduce $z\equiv e^{-\beta\epsilon}$. For $\beta\in\R$, $z>0$.
Then by the fundamental theorem of
algebra, we can rewrite $Z_N$
\begin{equation}
Z_N=\sum_{n=1}^M g(n)z^n = \kappa\prod_{n=1}^M\left(1-\frac{z}{z_n}\right),
\end{equation}
where $z_n\in\C$ are the zeros of $Z_N$ and $\kappa$ is an irrelevant
overall factor. By \equatref{eq:ZNsimple}, the fact
that $g(n)>0$ $\Forall n$ ensures $Z_N$ is strictly positive, and thus none of
...
f=\frac{\log(Z_N)}{N}.
\end{equation}
Plugging in our polynomial for $Z_N$, dropping the irrelevant constant factor
...
discontinuity on the real axis, and we learn the following important statement:

\begin{theorem}{Lee-Yang I}{}
There are no phase transitions at finite volume.
\end{theorem}

This theorem is completely generic to any lattice system whose partition
function can be cast in the form \eqref{eq:ZNsimple}. Now let's see what happens
