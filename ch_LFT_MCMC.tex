...
In \secref{sec:MCMCintro} we introduce MCMC simulations as they 
are applied to the project. \secref{sec:statana} summarizes
some of the tools needed to statistically analyze the generated data. 
The final \secref{sec:implement} provides 
...
\equatref{eq:ergodicity}\index{ergodic} is not too large. For example 
...
our method of choice, varies smoothly between~\equatref{eq:NR} and 
...
\subsection{Random number generators}\index{random number generator}
...
\begin{figure}
  ...
  \caption{Example generation of pseudorandom numbers using the linear
           ...
           $a$, $b$, and $m$. Image created by C. Schmidt.}
  \label{fig:RNG} 
\end{figure} 
