...
discrete form of \equatref{eq:maxent}. Suppose we have some event with $M$
...
distributed in $M$ boxes, each box representing an outcome. Thinking of the
units of probability as balls, each possible way of distributing the balls among
the boxes represents a different probability distribution. We want to impose no
preference for one probability distribution over another, so we treat every
possible distribution as equally likely. In the literature, this is playfully
...
Bayes's theorem gives us a way to estimate model uncertainty
