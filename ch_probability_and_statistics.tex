...
You already have a rough idea of probability: It is a way of saying whether you
think something will happen or not, while in general signaling that you are not
...
{\it Bayesian}\index{Bayesian!approach} approach in
\secref{sec:bayes}.
...
It is worth emphasizing that if $X$ and $Y$ are independent,
...
\end{equation}
This is the {\it error propagation formula} as it is usually stated. 
...
&\approx0.39,
\end{aligned}\end{equation}
which is notably quite different from \dimens{1} normal distribution;
...
$\hat{f}=f(\hat{x})$.\footnote{While this discussion is framed
in terms of estimating $f(\hat{x})$, the jackknife can be used
also for direct estimates of a sample mean; this will be demonstrated
in the first Example of the section. As we will discuss later, it
shines for $f(\hat{x})$ as it gives a versatile way to propagate
error.}
...
Here's how the jackknife method works: We throw away the 
first\footnote{This is also sometimes referred to as a
{\it remove-1} jackknife. One can generalize the procedure by throwing
away $m$ measurements at a time, a {\it remove-m} jackknife. The latter
is helpful to combat autocorrelation in time series, discussed in
\secref{sec:timeSeries}. A remove-$m$ jackknife is then preferable
to an unbinned, naive estimate of a mean coming from a time series, giving
a further use case beyond error propagation. Since I am always looking
at data from time series, the remove-$m$ jackknife is my go-to approach
for computing uncertainties.} measurement 
...
Next let us see how the jackknife lets us estimate bias.\index{jackknife!bias}
...
\begin{proposition}{}{Jbias}
  ...
  \end{proof}
\end{proposition}
Comparing the final steps of \propref{prp:bias} and \propref{prp:Jbias},
...
Again, \propref{prp:bias} tells us this is asymptotically unbiased.
...
correction is small, so that it is enough to simply round your
...
overfitting}\index{overfitting}; characteristics of overfitting can
include wiggles where there are no data}.

To try to keep a spline from inventing more structure than is sensible given the
data, one can add further constraints on the splines besides matching at the
knots. For example, a {\it natural spline}\index{spline!natural} asks that
...
\section{Statistical analysis of time series}\label{sec:timeSeries}
...
using the standard unbiased formula, i.e. using \equatref{eq:umv}, we would
...
\equatref{eq:IAC} shows there are $N/\tauint$ effectively independent
...
will describe another interpretation, the {\it Bayesian}\index{Bayesian!approach} 
interpretation. An excellent introduction to this approach
...
\end{equation}
and we refer to $I$ as {\it testable information}\index{information!testable}.
...
entropy}\index{principle!maximum entropy}, which asks that we select
pr by maximizing
\begin{equation}\label{eq:ShannonEntropy}
S=-\int \dd{x} \pr{x} \log \frac{\pr{x}}{m(x)}
\end{equation}
with {\it measure}\index{measure} $m$, fulfills our criteria.
...
discrete form of \equatref{eq:ShannonEntropy}. Suppose we have some event with $M$
...
approximation}\index{Stirling's approximation},
\begin{equation}
\log N!\approx N\log N-N,
\end{equation}
which is valid whenever $N$ is large, and using
\equatref{eq:entropyProbability}, we get
\begin{equation}
...
entropy}\index{entropy!information} or {\it Shannon
entropy}\index{entropy!Shannon} or {\it Kullback number}\index{Kullback number}
\begin{equation}
S\equiv  -\sum_{i=1}^M \pr{X_i}\log\pr{X_i}.
\end{equation}
Generalizing this to continuous variables gives us
\equatref{eq:ShannonEntropy}. We introduce a measure $m$ in this case, which
...
either, we need some kind of {\it model}\index{model}. Heuristically, a model is
...
For example in \chref{ch:preliminaries} we will introduce lattice gauge theory,
...
This is sometimes called {\it model uncertainty}\index{model!uncertainty}.
...
is known as {\it Bayesian model averaging}\index{Bayesian!model average} (BMA).


\begin{theorem}{}{BMAvar}
If one computes expectation values according to \equatref{eq:BMAmean},
...
within model $M$.
\begin{proof}
We have, according to \equatref{eq:BMAmean},
\begin{equation}\begin{aligned}
