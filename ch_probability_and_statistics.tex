...
model uncertainty with lattice field theory in mind. 
We assume the data set $D$ is described by some ``true" model $M_T$
We will try to describe our data with many models $M_\mu$.
The goal is to be able to obtain expectation values of functions $f$
of some model parameters $\vec{a}$. We will compute them as
\begin{equation}
\ev{f\left(\vec{a}\right)}=
\sum_\mu f\left(\vec{a}_\mu^*\right) \pr{M_\mu\given D},
\end{equation} 
where the sum runs over all attempted models, indexed by $\mu$, and  
$\vec{a}_\mu^*$ denotes the best-fit parameters for the model $M_\mu$.
