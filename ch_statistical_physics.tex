...
But again, from my perspective, it is more interesting to have this relation between
...
of equilibrium, which means it is quasistatic by definition.} changes
\begin{equation}
Q=T\dd{S}.
\end{equation}
...
\section{Conjugate variables and generalized forces}

Different control parameters like pressure and volume are introduced
to the energy as products of {\it conjugate variables}\index{conjugate}.
One is intensive, and one is extensive, so that the product remains extensive,
like the energy.\footnote{In \secref{sec:legendre} we will introduce
Legendre transformations. These are always formed by adding/subtracting a
product of conjugate variables, too.} For example $PV$ is a product of conjugate
variables. 

Sometimes, the pair is further specified as a {\it generalized
force}\index{generalized force} and a {\it generalized
displacement}\index{generalized displacement}. The force is the intensive part,
and the displacement is the extensive part. In the case of $PV$, this
association is clear, $P$ being directly related to a literal external force
applied to the system. Then $V$ changes due to the displacement of the
boundaries. But a generalized displacement can be, in principle, anything, which
is useful if you know there exists some control parameter besides $S$, $T$, $P$, 
or $V$ that affects the system's energy.

The first law of thermodynamics in terms of our most common control parameters
reads
\begin{equation}
  \dd{U}=T\dd{S}-P\dd{V}.
\end{equation}
The differential on the LHS is in an extensive variable, and accordingly, the
differentials on the RHS are in the generalized displacements.
In this context, consider the effect of adding particles of a particular
species. We can count the number of particles $N$, which is extensive and thus
our generalized displacement. If we want to introduce this to the energy, we
need a conjugate generalized force. For now, let's call that $\mu$. Then
the first law would be modified as 
\begin{equation}
  \dd{U}=T\dd{S}-P\dd{V}+\mu\dd{N},
\end{equation}
choosing the sign to be positive since adding a particle should increase the
energy. A simple perspective to take, then, is that this defines $\mu$; in
particular,
\begin{equation}
  \mu=\pdv{U}{N},
\end{equation}
and thus we can view $\mu$ as the change in energy per added particle.
We call $\mu$ the {\it chemical potential}\index{chemical potential}.
...
\end{equation}
where the sum runs over particle species,
...
\section{Legendre transforms}\label{sec:legendre}\index{Legendre transformation}
...
Hence we are looking for a transformation $L$ of $U$ such that
\begin{enumerate}
\item $LU$ is a function of $\partial_T U$ rather than a function of $T$; and 
\item we do not want to lose any information we had before, i.e. we want $L$ to
      be its own inverse.
\end{enumerate}
These are the purposes of a Legendre transformation. The second point 
may seem too obvious to state, but it is worth emphasizing here because 
...
Also to keep from losing information, it is convenient to demand $U$ is
monotonic in $T$ and $LU$ is monotonic in $\partial_T U$. That way there
is no ambiguity when mapping between $U$ and $LU$. Thankfully we know $U$
is monotonic in $T$.

Now let's think about a good approach to define $L$. Consider $f$ monotonic in
$x$. What would I need to reconstruct $f$? One way to construct it is if you
know the slope and $y$-intercept at every point; in particular, the function is the
envelope of its tangent lines. Let
\begin{equation}
p\equiv \partial_x f.
\end{equation}
The {\it Legendre transform} $L$ of $f$ is
\begin{equation}\label{eq:legendre}
Lf\equiv f-px.
\end{equation}
What is genius about this definition is that it encodes both the slope at each point
through $p$ and its intercept $f-px$. Moreover, $p$ was introduced in such a way
that $Lf$ is monotonic in $p$, guaranteeing we can meaningfully carry out the
operation $LLf(p)$. What do we find? Let
\begin{equation}
q\equiv \partial_p (Lf) = -x
\end{equation} 
Then
\begin{equation}\label{eq:legendre}
LLf= Lf-qp = (f-px)-(-x)p = f,
\end{equation}
as desired. So $Lf$ is our function depending on $\partial_x f$ with 
$L=L^{-1}$.
...
Legendre transform $U(S)$ out of the variable $S$, then 
the new independent variable is $T$. Calling
...
what is held fixed. For example $F(V,T,\vec{N})$, so the partial derivative in
...
\index{free energy!Gibbs}

One arrives at the {\it Gibbs} free energy using both the Helmholtz and enthalpy
Legendre transformations,
\begin{equation}
G(P,T,\vec{N})=U+PV-TS,
\end{equation}
and using the first law, we find
\begin{equation}
dG=V\dd{P}-S\dd{T}+\sum_i\mu_i\dd{N}_i.
\end{equation}
...
higher speed. Conversely if it is harder to move the constituents of the
...
To illustrate how to obtain such relations, we consider 
...
\equatref{eq:fslaw}. We consider $U(V,T,\vec{N})$; hence the exact differential 
...
\end{equation}
Comparing \equatref{eq:maxDeriv1} and \equatref{eq:maxDeriv2},
we can identify
...
\end{equation}
at fixed $\vec{N}$ to \equatref{eq:maxDeriv3} yields
\begin{equation}
\frac{1}{T}\atFixed{\pdv{C_V}{V}}{\vec{N}}=
\frac{\partial}{\partial T}
\frac{1}{T}\left[\atFixed{\pdv{U}{V}}{T,\vec{N}} +P\right].
\end{equation}
Using \equatref{eq:CV} for $C_V$ in the above equation and 
cancelling identical terms yields the {\it energy equation}\index{energy equation}\footnote{The energy equation is useful in that it allows one to
...
\end{equation}
At the same time, \propref{prp:chainconstrain} implies
\begin{equation}
...
\equatref{eq:TdS1} and \equatref{eq:TdS2} and taking the ratio
yields the first relation in \equatref{eq:matparamconstraints}.
The second relation is derived by equating \equatref{eq:TdS1} and 
\equatref{eq:TdS2} while holding $P$ and $\vec{N}$ fixed.
