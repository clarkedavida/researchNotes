...
The example code, shown in \\listref{lst:CATclass}, 
...
in \\listref{lst:CATmain}. The output of this code is
given in \\listref{lst:CATout}. In the first line of
...
\caption{Resulting output from main C++ code of \\listref{lst:CATmain}.
...
\caption{Output for the main code of \\listref{lst:scope}.
...
which is shown in \\listref{lst:CATpython}. One nice feature of Python classes
...
\caption{Output from the code of \\listref{lst:CATmain}.}
\label{lst:CATout}
\end{code*}
...
is given in \\listref{lst:CATmain} along with the
corresponding output in \\listref{lst:CATout}.
...
The code is shown in \\listref{lst:SU2}.
...
\caption{Output from \\listref{lst:SU2main}.}
\end{code*}
...
In \\listref{lst:SU2main} we give some quick examples of 
...
indicate that it is hexadecimal\footnote{Instead it is preceded
by \ff{0b} if it is binary.}.
...
support those FLOPs. From this equation it is clear that the attainable
FLOP/s scales linearly with arithmetic intensity.

\figref{fig:roofline} shows a sketch of this model. The vertical
...
catch undefined behavior; hence it is safest to just use the rule of three from
the beginning.
...
A strategy to help improve code reuse and flexibility is\index{policy!based design} {\it policy-based design}.
...
like what was discussed in \secref{sec:fitchi2}. Therefore we take as our
...
Very broadly we can imagine {\it artificial intelligence}\index{artificial intelligence} (AI) as trying to get computers to mimic human intelligence. 
A naive\footnote{Indeed I tried writing a ``chatbot" using QBASIC in this
manner when I was a little boy. This is why it is crucial to be specific
...
every\footnote{This makes sense since it is the most
...
There are many {\it universal approximation theorems}\index{universal approximation theorem} which guarantee that NNs converge to real-world
...
{\it filter}\index{filter} or {\it kernel}\index{kernel}. For a \dimens{2} image, the
kernel is a \dimens{2} grid, typically of size $3\times 3$, within which one
...
learning}\index{learning!contrastive}. Here one starts with an example $e$,
