...
In particular, when one normally regularizes in QFT, it is because
...
depicted in \figref{fig:links} (left).
...
\figref{fig:links} (right); alternatively one can view it
...
This proposition neglects all $\order{a^3}$ terms; I just wanted you to see
how this kind of relation can be shown. If one computes the plaquette
up to $\order{a^4}$, inserting the result into \equatref{eq:wilsonaction}
and expanding in $a$, one finds\footnote{The $\order{a^2}$ term dies
because we take the real part. Terms of $\order{a^p}$ with $p$ odd will
die in the sum because they come attached to terms with $p$ $\mu$ indices
and $p$ $\nu$ indices that are antisymmetric in $\mu$ and $\nu$.
For this reason, valid discretizations of QCD actions should contain
only even powers of $a$, as the QCD Lagrangian is parity-invariant.
This is worked out in detail in Section 1.5 of Ref.~\cite{Sciarra:2016exn}
...
%    is antisymmetric.
%    All terms of $\order{a^p}$ with $p$ odd will vanish in the sum.
%    To see this, note that each power of $a$ has some object with
%    exactly one $\mu$ and $\nu$ index attached to it.
%    In general, terms of $\order{a^p}$ will be attached
%    to products like $(F_{\mu\nu})^{p-2}$. When $p$ is odd, these terms will
%    vanish, since $F_{\mu\nu}$ to an odd power is antisymmetric under
%    interchange of $\mu$ and $\nu$, so the summation over $\mu<\nu$ will
%    kill them off.
%    \begin{equation*}
%    \begin{aligned}
...
%only even powers of $a$ are permitted in the expansion.
...
are the ones that are not measurable.}
...
It is perhaps instructive to contrast the QCD beta function with that of QED
in the continuum. At one loop, the QED beta function is
\begin{equation}
\beta_{\rm QED}(e)=\frac{e^3}{12\pi^2},
\end{equation}
which leads to, for two energy scales $q>\Lambda$,
\begin{equation}\label{eq:QEDbeta}
e(q)^2=\frac{e(\Lambda)^2}{1-e(\Lambda)^2
\log\left(q/\Lambda\right)/6\pi^2}.
\end{equation}
In this case, the coupling $e$ is an increasing function of $q$, which persists
up to the {\it Landau pole}\index{Landau pole} at
\begin{equation}
q=\Lambda\exp\left(\frac{6\pi^2}{e(\Lambda)^2}\right),
\end{equation}
where the coupling diverges. Since $\beta_{\rm QED}$ was determined
perturbatively, this pole exists well beyond the region for reliable
perturbative analysis; really it indicates only an eventual breakdown of this
approach. Thus if one were to take a continuum limit, one eventually reaches
energies where the beta function is no longer well defined.
...
\index{zero-temperature calculation}
...
Another perspective is that true $T=0$ calculations are achieved by simulating
...
\begin{figure}
  ...
  \includegraphics[width=0.5\textwidth]
          {figs/regge.pdf}
  \caption{Some example Regge trajectories. Image taken from
Ref.~\cite{Bali:2000gf}.}
  \label{fig:regge}
\end{figure}
...
In particular, a {\it Regge trajectory}\index{Regge trajectory} plots the spin
$J$ of a meson or baryon against its squared mass $m^2$~\cite{Regge:1959mz}. 
It has been noticed since the early 1960s that such plots tend to be nearly 
linear. The relativistic string model~\cite{Goto:1971ce,Goddard:1973qh}
predicts a relation\footnote{Of course, this is
where the string tension gets its name from.} 
\begin{equation}\label{eq:regge}
J=J_0+\frac{m^2}{2\pi\sigma};
\end{equation}
therefore one can take a plot like \figref{fig:regge} and extract $\sigma$
using \equatref{eq:regge} from the slope. Doing so yields estimates
\begin{equation}
410~{\rm MeV}\lesssim\sqrt{\sigma}\lesssim 470~{\rm MeV},
\end{equation}
so not very precise, as stated before. 
For more detailed discussion about the string tension, see e.g.
Ref.~\cite{Bali:2000gf}.
...
can be seen for example in \figref{fig:ploopScatter}. I've been told
...
One can see $\ev{y^2}\neq0$ in \figref{fig:ploopScatter}.
...
\end{equation}
for some volume-independent constant $A$. Indeed \figref{fig:ImPvol} shows 
