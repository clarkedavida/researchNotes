\chapter{LGT: The Basics}\label{ch:preliminaries}
...
We will also use {\it natural units}\index{natural units}
...
We close this subsection with a remark about confinement.
...
a distance $R$. The {\it string tension}\index{string tension} 
$\sigma$ is defined by
\begin{equation}
...
\end{equation}
and the integration measure, called the {\it Haar}\index{Haar measure} is
\begin{equation}
...
\section{Reference scales}\label{sec:refscales}\index{scale setting}
...
The order parameter for this transition is the \index{Polyakov loop}
{\it Polyakov loop}, 
\begin{equation}
...
\begin{equation}
  y=t_c^2 E(t_c),
\end{equation}
and a {\it cooling scale}\index{cooling!scale} is given by
...
as \index{asymptotic scaling}{\it asymptotic scaling}.
   ...
   \var n &=-\frac{1}{24\pi^2}~\var\int_\Omega\dd[3]{x}\epsilon_{ijk}\tr
            ...
            &=+\frac{1}{8\pi^2}\int_\Omega \dd[3]{x}\epsilon_{ijk}\tr
                \partial_i\left(U^\dagger\var U\right)
                 U^\dagger\partial_jU\,
                 U^\dagger\partial_kU\\
            &=+\frac{1}{8\pi^2}\int_{\partial\Omega} \dd{S_i}\epsilon_{ijk}\tt
r
                U^\dagger\var U\,
  ...
  \end{align*}
  In the second step we used that the trace is cyclic.
  ...
  evaluated at infinity. Since $\partial_jU=\partial_jU_0=0$ there, this termm
...
By viewing the integrand of eq.~\eqref{eq:wn} 
...
With eq.~\eqref{eq:Q} we can find vacuum solutions to the Euclidean
field equations
\begin{equation}
  D_\mu F_{\mu\nu}=0.
\end{equation}
...
\section{The Polyakov loop}\index{Polyakov loop}
...
At higher temperatures the first equality of eq.~\eqref{eq:centerzero}
