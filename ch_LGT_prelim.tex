...
Using eq.~\eqref{eq:haarcommute} it follows for any 
...
\end{proposition}
In Sec.~\ref{sec:hqfe} we will encounter products of Wilson
loops that have one link in common. The third formula in
Prop.~\ref{prp:haar} delivers the following equation, allowing
...
The Coulomb-like term in eq.~\eqref{eq:staticpotential} can be seen
mnemonically as follows. From eq.~\eqref{eq:fieldstrength}
...
\begin{equation}\begin{gathered}
    \exp\left(\frac{\beta}{6}\sum_\square\left(\tr \plaq+\tr
     \plaq^{\,\dagger}\right)\right)~~~~~~~~~~~~~~~~~~~~
       ~~~~~~~~~~~~\\
    =\sum_{i,j=0}^{\infinity}\frac{1}{i!j!}\left(\frac{\beta}{6}\right)^{i+j}
      \left(\sum_\square\tr\plaq\right)^i\left(\sum_{\square'}\tr\plaq^{'\,\dagger}\right)^j
...
The order parameter\footnote{We will argue this in Sec.~\ref{sec:ploop}.} 
