...
or {\it members}\index{member}, and we indicate that, e.g. 1 is an element of
$A$ by writing
\begin{equation}
1\in A,
\end{equation}
...
b\notin A,
\end{equation}
The number of elements in a finite set is called its {\it cardinality}\index{cardinality}. The cardinality of the set $A$ above is 4, and we write
\begin{equation}
  |A|=4.
\end{equation}
A {\it finite} set has a finite cardinality; otherwise it is an infinite set.
...
\end{equation}
The {\it empty set}\index{set!empty} $\emptyset$ is the set with nothing in it
at all. If a set has at least one element, it is {\it nonempty}\index{nonempty}.
...
A {\it function}\index{function} or {\it mapping}\index{map} $f$ between $X$ and $Y$
associates $\Forall x\in X$ a unique element $f(x)\in Y$. We
write
\begin{equation}
f:X\to Y.
\end{equation}
Usually $X$ is called the {\it domain}\index{domain}
and $Y$ is called the {\it codomain}\index{codomain}.
The {\it image}\index{image} or {\it range}\index{range}
...
A function is {\it injective}\index{injection}
or {\it one-to-one}\index{one-to-one} if each element in $X$
maps to a different element in $Y$. We can express this symbolically as
\begin{equation}
 \Forall x_1,x_2\in X,~f(x_1)=f(x_2)\Rightarrow x_1=x_2.
\end{equation}
A {\it surjective}\index{surjection} or {\it onto}\index{onto} function
...
least one" or ``there exists". Finally a function is {\it bijective}\index{function!bijective} if it is both an injection and a surjection. In this case each
...
It is actually not too common for mathematicians or physicists to write the
...
\end{equation}
is called an {\it equivalence class}\index{equivalence class}.
...
Additionally it is clear that each $G_i$ can be considered
a subgroup of the direct product. For instance $G_1$ is
isomorphic to the group of tuples
\begin{equation}
(g_1,\id,...,\id).
\end{equation}
It is also useful to note that the direct product is
...
\begin{proposition}{}{cosetPartition}
  ...
  \begin{proof}
    ...
    \begin{enumerate}
      \item Let $h\in H$. Then $g=h(h^{-1}g)$, i.e. it is in the right
            coset of $H$. A similar computation shows it is in the left
            coset of $H$ as well. 
...
\index{modular arithmetic}
  ...
  normal subgroup of $\Z$ since it is commutative. To form the 
...
\section{Vectors and vector spaces}\label{sec:vectors}
  ...
  less formal and refer to the underlying field only if it is absolutely
  necessary.
...
The {\it range}\index{range} or {\it column space} of $M$ is the vector space
spanned by its column vectors.
Then the {\it rank}\index{rank} of $M$ is the dimension of the 
...
\begin{equation}
  \zd_n=\left(\begin{array}{cccc}
          ...
          0   &   &       & 0
            \end{array}\right), 
\end{equation}
i.e. it is the $n\times n$ matrix with 0 in every element. With our definition
...
A {\it Hankel matrix}\index{matrix!Hankel} $H$ is any $n\times n$ matrix of the form
\begin{equation}
H=\begin{pmatrix}
  a_1 & a_2 & a_3 &  & a_n  \\
  a_2 & a_3 &     &  & \\
...
{\it Schur decomposition}\index{Schur decomposition}.
...
We wrap up with a theorem that is useful to derive expressions relating
...
It is useful to use alternative algorithms whenever we can find them,
...
since it is ``almost not invertible".
...
\begin{equation}
\mat{\pdv{A}{x}}{ij}\equiv\pdv{A_{ij}}{x},
\end{equation}
...
Knowing how to do matrix-by-scalar and scalar-by-matrix derivatives
...
A more general object is a {\it tensor}\index{tensor} $T$, which you can think
of as a ``matrix" indexed by $n\in\N$ indices. The number of indices $n$
is called the {\it rank}\index{rank} of the tensor. One way of thinking of
...
we write tensor products element-wise as\index{product!tensor}
\begin{equation}
...
Recall from \secref{sec:gpprelim} the 
permutation group\index{group!permutation}\index{group!symmetric} on
...
$S_n$. We can write any element in $S_n$ as a {\it cycle}\index{cycle}, 
...
A simple $n$-dimensional representation of $S_n$ permutes the orthonormal
...
\end{equation}
This is the {\it defining representation}\index{representation!defining}
of $S_n$.

A set $C$ is called a {\it conjugacy class}\index{conjugacy class} if 
$gCg^{-1}=C$. If you have a group element $h$, the 
{\it conjugation}\index{conjugation} of $h$ is $ghg^{-1}$.
...
is\index{quadratic Casimir}\index{Casimir!quadratic}
\begin{equation}\label{eq:quadCasimir}
C_2\equiv R^a R^a.
\end{equation}
...
satisfies $a_\mu a_\mu=1$, which means it is a point on the surface of a 4D
...
\subsection{Some useful facts for groups}
